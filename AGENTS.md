...
- `src/chat-server-cli.ts` serves the `marshall.chat` gateway. In `--runtime p2p_worker` mode it serves `chat/public`, accepts `--p2p-worker-addr` plus comma-separated `--p2p-worker-addrs`, exposes `GET /api/health`, `GET /api/inference/workers`, `GET /api/conversation?conversation_id=<id>`, `POST /api/conversation/memory`, `POST /api/chat`, and SSE `POST /api/chat/stream`, persists conversation turns, builds a bounded context window, and forwards prompts through the inference router. Use this for real distributed inference tests. `local_process` mode remains available only for single-machine debugging.
...
- `chat/public/app.js` submits the composer on `Enter`, preserves multiline input on `Shift+Enter`, uses `POST /api/chat/stream` for streamed answers, lets the user edit persisted long-term memory/plans, and displays inference worker readiness plus the last selected worker without exposing raw worker multiaddrs.
...
- `tests/chat-memory.test.ts` verifies persistent conversation storage, safe conversation IDs, and context trimming. `tests/chat-server.test.ts` verifies local chat streaming and persisted streamed conversations. `tests/inference-p2p.test.ts` verifies the distributed chat path with real libp2p peers, including worker capability probes, P2P streaming, and router failover across compatible inference workers. Run P2P tests with Node.js 22+ outside restricted sandboxes because they open TCP sockets.
