...
- `scripts/check-chat-public.sh` is the readiness gate for public chat inference. It requires explicit `--url` and `--expected-ip`, verifies DNS, HTTPS health, one streamed P2P response, at least one ready worker, and checks that public `completed` SSE events do not leak the gateway-composed prompt. Run it against `https://marshall.training/chat/` for the fallback and `https://marshall.chat` before declaring the chat domain operational.
