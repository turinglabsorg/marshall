...
- `src/model-promote-cli.ts` is the worker-side promotion path for ready models. It requires explicit `--control`, `--package-job-id`, and `--package-hash`, fetches the package and adapter from the control peer via `/marshall/artifact/fetch/1.0.0`, verifies every chunk/file/root hash through `src/artifact-transfer.ts`, then writes a local `model_package.json` whose `adapter_path` points at the fetched local adapter. Use this before starting an inference worker on a model finalized by another machine.
...
- `src/inference-worker-cli.ts` starts a libp2p inference worker for `marshall.chat`. The worker owns the base model + adapter paths, handles `/marshall/inference/hello/1.0.0` capability probes, `/marshall/inference/generate/1.0.0`, and `/marshall/inference/generate_stream/1.0.0`, and runs `training/mlx_lora_chat.py` locally to return generation responses or stream events.
- `src/inference-router.ts` is the gateway-side inference router. It probes configured worker multiaddrs, filters workers by exact model + adapter id + adapter hash, exposes non-sensitive worker snapshots, selects the least-busy compatible worker, streams JSONL events when workers support `generate_stream`, and falls back across compatible workers when generation fails.
...
- `chat/public/app.js` submits the composer on `Enter`, preserves multiline input on `Shift+Enter`, uses `POST /api/chat/stream` for streamed answers, lets the user edit persisted long-term memory/plans, and displays inference worker readiness, ready model packages, and the last selected worker without exposing raw worker multiaddrs.
