...
- `src/chat-server-cli.ts` serves the `marshall.chat` gateway. In `--runtime p2p_worker` mode it serves `chat/public`, exposes `GET /api/health`, `GET /api/conversation?conversation_id=<id>`, and `POST /api/chat`, persists conversation turns, builds a bounded context window, and forwards prompts over libp2p to `--p2p-worker-addr`. Use this for real distributed inference tests. `local_process` mode remains available only for single-machine debugging.
- `chat/public/app.js` submits the composer on `Enter` and preserves multiline input on `Shift+Enter`.
- `tests/chat-memory.test.ts` verifies persistent conversation storage, safe conversation IDs, and context trimming. `tests/inference-p2p.test.ts` verifies the distributed chat path with two real libp2p peers: an HTTP gateway peer and an inference worker peer. Run P2P tests with Node.js 22+ outside restricted sandboxes because they open TCP sockets.
