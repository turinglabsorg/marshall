...
- `coordinator/public/index.html` is the embedded terminal-style swarm console for worker status, job status, published-work completion, per-worker progress, cluster speed, artifact verdicts, and live coordinator events. The completion panel is computed from coordinator jobs and reports terminal/total progress across training, evaluation, and validation phases; phases with no published jobs must show as not scheduled rather than inventing hidden work. The event stream must display compact identifiers only and avoid full worker IDs, full peer IDs, raw messages, or other sensitive/debug-heavy payloads.
- `coordinator/public/index.html` groups coordinator jobs by canonical run. Active runs drive the top contract, completion panel, and jobs table by default; completed historical runs stay hidden until the user clicks `show history`. The jobs table must paginate visible rows in pages of 6, and workers plus event stream should remain a 50/50 live grid on desktop.
