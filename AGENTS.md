...
- `src/inference-worker-cli.ts` starts a libp2p inference worker for `marshall.chat`. The worker owns the base model + adapter paths, handles `/marshall/inference/hello/1.0.0` capability probes and `/marshall/inference/generate/1.0.0`, and runs `training/mlx_lora_chat.py` locally to return a generation response.
- `src/inference-router.ts` is the gateway-side inference router. It probes configured worker multiaddrs, filters workers by exact model + adapter id + adapter hash, exposes non-sensitive worker snapshots, selects the least-busy compatible worker, and falls back across compatible workers when generation fails.
...
- `src/chat-server-cli.ts` serves the `marshall.chat` gateway. In `--runtime p2p_worker` mode it serves `chat/public`, accepts `--p2p-worker-addr` plus comma-separated `--p2p-worker-addrs`, exposes `GET /api/health`, `GET /api/inference/workers`, `GET /api/conversation?conversation_id=<id>`, and `POST /api/chat`, persists conversation turns, builds a bounded context window, and forwards prompts through the inference router. Use this for real distributed inference tests. `local_process` mode remains available only for single-machine debugging.
- `chat/public/app.js` submits the composer on `Enter`, preserves multiline input on `Shift+Enter`, and displays inference worker readiness plus the last selected worker without exposing raw worker multiaddrs.
- `tests/chat-memory.test.ts` verifies persistent conversation storage, safe conversation IDs, and context trimming. `tests/inference-p2p.test.ts` verifies the distributed chat path with real libp2p peers, including worker capability probes and router failover across compatible inference workers. Run P2P tests with Node.js 22+ outside restricted sandboxes because they open TCP sockets.
