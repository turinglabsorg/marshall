...
- `src/round-daemon-cli.ts` is the product path for unattended round advancement. It polls coordinator jobs/artifacts, waits for train/eval/validation phase completion, writes filtered artifact snapshots for the current run, invokes the round orchestrator, and stops after model selection. Use it instead of manually switching the public control peer or hand-running phase commands.
...
- The dashboard shows worker status, job status, active-run completion, per-worker progress, cluster speed, artifact verdicts, and live coordinator events. The completion panel is computed from the active run only and reports terminal/total progress across training, evaluation, and validation phases; phases with no published jobs must show as not scheduled rather than inventing hidden work. The event stream must display compact identifiers only and avoid full worker IDs, full peer IDs, raw messages, or other sensitive/debug-heavy payloads.
