...
- `src/e2e-ag-news-cli.ts` runs the AG News product E2E path: training worker pool, evaluation worker pool, validation worker pool, coordinator quorum checks, leaderboard, package, query, and optional coordinator persistence verification. Use `--model-backend text_classifier` for the stdlib CPU path and `--model-backend mlx` for LoRA. The runner accepts `--job-prefix` or `--job-id` and fails if leaderboard/model selection contains artifacts outside the current job prefix.
