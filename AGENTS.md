...
- `src/chat-memory.ts` implements durable gateway-owned chat memory with file-backed `conversation_id` records under `.marshall/chat/conversations` by default. This memory includes message history, model package metadata, and a summary field for future compaction. Inference workers must remain stateless and must not store private chat history.
- `src/chat-server-cli.ts` serves the `marshall.chat` gateway. In `--runtime p2p_worker` mode it serves `chat/public`, accepts `--p2p-worker-addr` plus comma-separated `--p2p-worker-addrs`, exposes `GET /api/health`, `GET /api/inference/workers`, `GET /api/conversation?conversation_id=<id>`, `POST /api/chat`, and SSE `POST /api/chat/stream`, persists conversation turns, builds a bounded context window, and forwards prompts through the inference router. Use this for real distributed inference tests. `local_process` mode remains available only for single-machine debugging.
- `chat/public/app.js` submits the composer on `Enter`, preserves multiline input on `Shift+Enter`, uses `POST /api/chat/stream` for streamed answers, and displays inference worker readiness plus the last selected worker without exposing raw worker multiaddrs.
