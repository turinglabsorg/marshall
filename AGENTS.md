...
- `src/round-orchestrator-cli.ts` is the product path for advancing a run phase. It reads coordinator artifacts or an artifact snapshot and can schedule evaluation jobs from LoRA artifacts, schedule validation jobs from unvalidated adapter-evaluation artifacts, or produce accepted-only leaderboard/model-package outputs after validation finalizes. It returns the next `control.job_type` and `control.jobs_file` to serve. Prefer this over manual eval/validation/leaderboard command sequences when operating a round.
